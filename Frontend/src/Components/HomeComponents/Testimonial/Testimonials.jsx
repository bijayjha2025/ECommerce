...
export default function Testimonials() {
  ...
  return (
   <section className="bg-[#523902] py-20 px-6 my-16 relative overflow-hidden">
    ...
    <div className="max-w-[720px] mx-auto text-center relative z-10">
     ...
     <div style={{ opacity: animating ? 0 : 1, transform: animating ? `translateX(${direction === 'next' ? '-24px' : '24px'})` : 'translateX(0)', transition: 'opacity 0.35s ease, transform 0.35s ease', }}>
      <p className="font-serif text-[clamp(1.05rem,2vw,1.3rem)] leading-[1.8] text-[#1a0a00]/75 italic mb-8 tracking-wide">"{t.quote}"</p>
      ...
      <div className="flex flex-col items-center gap-1">
       <p className="font-serif text-[13px] text-[#1a0a00] tracking-wide">{t.name}</p>
       <p className="font-serif text-[10px] tracking-[0.2em] uppercase text-[#b8860b]/60">{t.location}</p>
      </div>
     </div>
